    ...
    module IssuesControllerPatch
      ...
      module InstanceMethods
        def update_with_check_spent_time
          ...
          current_project = Project.find(params[:issue][:project_id]) #Issue.find(params[:issue]).project
          current_status = params[:issue][:status_id] 
          if ((!params[:time_entry].nil?) && (!Issue.find(params[:id]).children?))
            if ((params[:time_entry][:hours] == "") && (allowed_projects.member?(current_project.id.to_s)) && (allowed_statuses.member?(current_status.to_s)))
              find_issue
